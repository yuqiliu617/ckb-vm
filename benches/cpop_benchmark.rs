...
#[cfg(has_asm)]
fn bench_cpop(c: &mut Criterion) {
    let buffer = std::fs::read("tests/programs/cpop_microbench")
        .unwrap()
        .into();
    c.bench_function("cpop_microbench", |b| {
        b.iter(|| vm_bench_utils::run_program(&buffer, ISA_IMC | ISA_B, VERSION1))
    });
}

#[cfg(has_asm)]
criterion_group!(benches, bench_cpop);
#[cfg(not(has_asm))]
